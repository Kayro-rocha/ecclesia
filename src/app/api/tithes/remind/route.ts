...
export async function POST(req: NextRequest) {
  ...
  const fcmMembers = await prisma.member.findMany({
    where: { id: { in: targetMemberIds }, fcmToken: { not: null } },
    select: { fcmToken: true },
  })
  const fcmTokens = fcmMembers.map(m => m.fcmToken!)
  let fcmSent = 0
  if (fcmTokens.length > 0) {
    const result = await sendFcmToMany(fcmTokens, title, message)
    fcmSent = result.sent
  ...
  return NextResponse.json({ ok: true, sent: sent + fcmSent, total: subscriptions.length })
}
