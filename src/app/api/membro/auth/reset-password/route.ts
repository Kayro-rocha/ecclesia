...
export async function POST(req: NextRequest) {
  ...
  const record = await prisma.memberResetToken.findUnique({ where: { token } })
  ...
  await prisma.member.update({ where: { id: record.memberId }, data: { memberPassword: hashed } })
  await prisma.memberResetToken.update({ where: { token }, data: { usedAt: new Date() } })
