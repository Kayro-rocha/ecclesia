...
export default function EscalaActions({ slug, escalaId, foiEnviada, temVoluntarios, title, date, department, membros }: Props) {
  ...
  async function handleNotify() {
    ...
    if (!await confirm(`Enviar notificação push para os membros sobre a escala "${title}"?`, { title: 'Enviar notificação', confirmText: 'Enviar' })) return
  ...
  async function handleDelete() {
    ...
    if (!await confirm(msg, { title: 'Apagar escala', confirmText: 'Apagar', variant: 'danger' })) return
